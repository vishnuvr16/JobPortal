    ...
    const fetchJobDetails = async () => {
      if (isEditMode) {
        try {
          ...
          setValue('salaryMin', jobData.salaryRange.min);
          setValue('salaryMax', jobData.salaryRange.max);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const url = isEditMode 
        ? `${summaryApi.defaultUrl}/jobs/${id}` 
        : `${summaryApi.defaultUrl}/jobs`;
      ...
      if (!response.ok) {
        throw new Error(`Failed to ${isEditMode ? 'update' : 'add'} job`);
      }

      alert(`Job ${isEditMode ? 'updated' : 'added'} successfully!`);
      navigate('/admin'); // Redirect to dashboard after successful operation
    } catch (error) {
      console.error(error);
